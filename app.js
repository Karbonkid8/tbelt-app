...
function wellModal(id, selectedSiteId) { const well = (state.adminWells || []).find(item => item.id === id); const siteId = well?.siteId || selectedSiteId; const selectedSite = state.sites.find(site => site.id === siteId); const title = well ? `Edit ${well.name}` : 'Add well'; const locationField = siteId ? `<label class="field">LOCATION<span class="locked-location">${escapeHtml(selectedSite?.name || siteId)} · ${escapeHtml(siteId)}</span><input type="hidden" name="siteId" value="${escapeHtml(siteId)}" /></label>` : `<label class="field">LOCATION<select required name="siteId">${state.sites.filter(site => site.active).map(site => `<option value="${escapeHtml(site.id)}" ${well?.siteId === site.id ? 'selected' : ''}>${escapeHtml(site.name)} · ${escapeHtml(site.id)}</option>`).join('')}</select></label>`; return modal(`<h2>${escapeHtml(title)}</h2><p>Wells are available to CNG users at their assigned location.</p><form id="well-form" data-id="${escapeHtml(well?.id || '')}">${locationField}<label class="field">WELL NAME<input required name="name" value="${escapeHtml(well?.name || '')}" placeholder="Example: Anthem 14-23H" /></label><label class="field">WELL COLOR<select name="color">${['green','blue','orange','purple','red','teal'].map(color => `<option value="${color}" ${well?.color === color ? 'selected' : ''}>${color[0].toUpperCase() + color.slice(1)}</option>`).join('')}</select></label><label class="field">PLANNED STAGES<input required name="plannedStages" type="number" min="1" step="1" value="${escapeHtml(well?.plannedStages || '')}" placeholder="42" /></label><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">${well ? 'Save well' : 'Add well'}</button></div></form>`); }
function manageWellsModal(siteId) { const site = state.sites.find(item => item.id === siteId); const wells = (state.adminWells || []).filter(well => well.siteId === siteId); return modal(`<h2>${escapeHtml(site?.name || siteId)} wells</h2><p>Set up and manage the wells assigned to this location.</p><div class="manage-wells-list">${wells.length ? wells.map(well => `<div class="manage-well-item"><div><b><span class="well-dot well-${escapeHtml(well.color)}"></span>${escapeHtml(well.name)}</b><span>${well.plannedStages} planned stages · ${well.active ? 'Active' : 'Inactive'}</span></div><button class="link-button" type="button" data-manage-edit-well="${escapeHtml(well.id)}">Edit</button></div>`).join('') : '<div class="muted">No wells have been added to this location yet.</div>'}</div><div class="modal-actions"><button type="button" class="secondary" data-close>Done</button><button class="primary" type="button" data-add-well-for="${escapeHtml(siteId)}">+ Add well</button></div>`); }
