...
function historyModal(id) { const container = state.containers.find(item => item.id === id); const latest = container.history?.[0]; return modal(`<h2>${escapeHtml(container.name)} history</h2><p>${escapeHtml(container.chemical)} · ${container.area}${latest ? ` · Latest ${formatStrap(latest.strap)}` : ''}</p>${historyChart(container)}<div class="history"><h3>Recorded readings</h3>${(container.history || []).slice(0, 8).map(item => `<div class="history-item"><b>${formatStrap(item.strap)}</b><span>${escapeHtml(item.at)} · ${escapeHtml(item.by)}</span></div>`).join('') || '<div class="muted">No previous strap readings.</div>'}</div><div class="modal-actions"><button type="button" class="primary" data-close>Done</button></div>`); }
