...
const seed = {
  ...
  containers: [
    { id:'iso-014', name:'ISO #014', type:'ISO tank', area:'Frac', chemical:'Friction Reducer', strap:56, updatedAt:'Today, 9:42 AM', history:[{strap:56, by:'Demo operator', at:'Today, 9:42 AM'}] },
    { id:'poly-05', name:'Poly #05', type:'Poly 330 gal', area:'Frac', chemical:'Scale Inhibitor', strap:31, updatedAt:'Aug 27', history:[{strap:31, by:'Demo operator', at:'Aug 27'}] },
    { id:'iso-021', name:'ISO #021', type:'ISO tank', area:'Pump Down', chemical:'Biocide', strap:48.5, updatedAt:'Yesterday', history:[{strap:48.5, by:'Demo operator', at:'Yesterday'}] },
    { id:'poly-08', name:'Poly #08', type:'Poly 330 gal', area:'Pump Down', chemical:'Corrosion Inhibitor', strap:null, updatedAt:'Not entered', history:[] },
  ],
};
...
function adminDashboard() { return `<section><div class="heading"><div><h1>Administrator console</h1><p class="subhead">Manage FieldOps locations and access.</p></div></div><section class="panel"><div class="panel-heading"><strong>Locations</strong><span>${state.sites.length} active</span></div>${state.sites.length ? `<table><thead><tr><th>Location</th><th>Status</th><th>Location ID</th></tr></thead><tbody>${state.sites.map(site => `<tr><td><b>${escapeHtml(site.name)}</b></td><td><span class="badge">${site.active ? 'Active' : 'Inactive'}</span></td><td class="muted">${escapeHtml(site.id)}</td></tr>`).join('')}</tbody></table>` : '<div class="empty">No locations are available.</div>'}</section></section>`; }
...
function row(container) { return `<tr><td><div class="container-cell">${typeIcon(container.type)}<b>${escapeHtml(container.name)}</b></div></td><td><span class="badge ${container.type === 'ISO tank' ? '' : 'poly'}">${container.type}</span></td><td>${escapeHtml(container.chemical)}</td><td class="strap">${formatStrap(container.strap)}</td><td class="muted">${container.updatedAt}</td><td><button class="link-button" data-update="${container.id}">${container.strap === null ? 'Enter strap' : 'Update'}</button></td></tr>`; }
