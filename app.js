...
const seed = {
  ...
  cng: {
    ...
    wells: [
      { id:'well-green', name:'Anthem 14-23H', alias:'Green', color:'green', plannedStages:42, active:true },
      { id:'well-blue', name:'Anthem 14-24H', alias:'Blue', color:'blue', plannedStages:38, active:true },
    ],
    trailers: [
...
function cng() {
  ...
  return `<section class="cng-page"><div class="heading"><div><div class="title-row"><h1>CNG</h1><span class="well-pill well-${escapeHtml(well.color)}">${escapeHtml(well.alias)}</span></div><p class="subhead">Quick trailer readings and end-stage gas volume.</p></div><div class="cng-well-controls"><label>ACTIVE WELL<select data-cng-well>${cngWells().map(item => `<option value="${escapeHtml(item.id)}" ${item.id === well.id ? 'selected' : ''}>${escapeHtml(item.alias)} · ${escapeHtml(item.name)}</option>`).join('')}</select></label><span class="stage-progress">Stage ${stageNumber} of ${well.plannedStages}</span><button class="secondary" data-stage-history type="button">Stage history</button></div></div><div class="cng-layout"><section class="panel cng-readings"><div class="panel-heading"><strong>Location trailer readings</strong><div><span>PSI required · °F optional</span><button class="link-button" data-manage-trailers type="button">Manage trailers</button></div></div>${trailers.length ? `<form id="cng-readings-form"><div class="cng-reading-labels"><span>POSITION / TRAILER</span><span>PSI</span><span>°F</span></div><div class="cng-reading-list">${trailers.map(trailer => `<div class="cng-reading-row"><b><span class="position">${trailer.position}</span> Trailer ${escapeHtml(trailer.trailerNumber)}</b><label><input name="psi-${trailer.id}" data-cng-psi="${trailer.id}" type="number" min="0" step="1" inputmode="decimal" placeholder="Enter" /><span>PSI</span></label><label><input name="temp-${trailer.id}" data-cng-temp="${trailer.id}" type="number" step="0.1" inputmode="decimal" placeholder="Optional" /><span>°F</span></label></div>`).join('')}</div><div class="cng-save-row"><span>Only changed readings are saved together with the current well.</span><button class="primary" type="submit">Save readings</button></div></form>` : '<div class="empty">No active trailers. Use Manage trailers to add the location lineup.</div>'}</section><section class="panel cng-stage"><div class="panel-heading"><strong>End stage</strong><span>Records ${escapeHtml(well.alias)}</span></div><form id="end-stage-form" class="cng-stage-form"><label class="field stage-number-field">STAGE NUMBER<input required name="stageNumber" type="number" min="1" step="1" value="${stageNumber}" /><span>of ${well.plannedStages} · Edit if needed</span></label><label class="field">MSCF PULLED THIS STAGE<input required name="mscf" type="number" min="0" step="0.1" inputmode="decimal" placeholder="Enter total MSCF" /></label><label class="field">NOTE <span class="optional">Optional</span><textarea name="note" maxlength="280" placeholder="Stage-end note"></textarea></label><button class="cng-end-button" type="submit">Review end stage</button><p class="cng-help">The end time is saved automatically. Volume after this entry rolls into the next stage.</p></form></section></div><section class="panel cng-trend"><div class="panel-heading"><strong>Trailer pressure trend</strong><div class="cng-trend-controls"><span>${escapeHtml(well.alias)} stage-end markers</span><div class="range-selector">${[12,24,36,48].map(hours => `<button data-cng-range="${hours}" class="${state.cng.rangeHours === hours ? 'active' : ''}" type="button">${hours}h</button>`).join('')}</div></div></div>${cngPressureChart(well, trailers)}</section><p class="cng-footnote">${completed} of ${well.plannedStages} stages completed for ${escapeHtml(well.alias)}.</p></section>`;
}
...
function wellModal(id) { const well = (state.adminWells || []).find(item => item.id === id); const title = well ? `Edit ${well.alias}` : 'Add well'; return modal(`<h2>${escapeHtml(title)}</h2><p>Wells are available to CNG users at their assigned location.</p><form id="well-form" data-id="${escapeHtml(well?.id || '')}"><label class="field">LOCATION<select required name="siteId">${state.sites.filter(site => site.active).map(site => `<option value="${escapeHtml(site.id)}" ${well?.siteId === site.id ? 'selected' : ''}>${escapeHtml(site.name)} · ${escapeHtml(site.id)}</option>`).join('')}</select></label><label class="field">WELL NAME<input required name="name" value="${escapeHtml(well?.name || '')}" placeholder="Example: Anthem 14-23H" /></label><label class="field">ALIAS<input required name="alias" value="${escapeHtml(well?.alias || '')}" placeholder="Example: Green" /></label><label class="field">WELL COLOR<select name="color">${['green','blue','orange','purple','red','teal'].map(color => `<option value="${color}" ${well?.color === color ? 'selected' : ''}>${color[0].toUpperCase() + color.slice(1)}</option>`).join('')}</select></label><label class="field">PLANNED STAGES<input required name="plannedStages" type="number" min="1" step="1" value="${escapeHtml(well?.plannedStages || '')}" placeholder="42" /></label><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">${well ? 'Save well' : 'Add well'}</button></div></form>`); }
...
function endStageConfirmationModal(data) { return modal(`<h2>End Stage ${data.stageNumber}?</h2><p>Confirm this stage-end record before it is saved.</p><div class="confirmation-list"><div><span>WELL</span><b>${escapeHtml(data.well.alias)} · ${escapeHtml(data.well.name)}</b></div><div><span>MSCF PULLED</span><b>${data.mscf}</b></div><div><span>END TIME</span><b>${escapeHtml(formatDateTime(data.endedAtIso))}</b></div></div><form id="confirm-end-stage-form" data-stage="${data.stageNumber}" data-mscf="${data.mscf}" data-note="${escapeHtml(data.note)}" data-ended-at="${data.endedAtIso}"><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">Confirm end stage</button></div></form>`); }
function stageHistoryModal() { const well = selectedWell(); const stages = [...stagesForWell(well?.id)].reverse(); return modal(`<h2>${escapeHtml(well?.alias || 'Well')} stage history</h2><p>${escapeHtml(well?.name || '')} · ${stages.length} of ${well?.plannedStages || 0} completed</p><div class="stage-history-list">${stages.length ? stages.map(stage => `<div class="stage-history-item"><div><b>Stage ${stage.stageNumber} · ${stage.mscf} MSCF</b><span>${escapeHtml(stage.endedAt)}${stage.note ? ` · ${escapeHtml(stage.note)}` : ''}</span></div><button class="link-button" data-edit-stage="${escapeHtml(stage.id)}">Edit</button></div>`).join('') : '<div class="muted">No stages have been completed for this well.</div>'}</div><div class="modal-actions"><button class="primary" type="button" data-close>Done</button></div>`); }
...
function bindModal() {
  ...
  document.querySelector('#well-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const well = { id:form.dataset.id || crypto.randomUUID(), siteId:String(data.get('siteId')), name:String(data.get('name')).trim(), alias:String(data.get('alias')).trim(), color:String(data.get('color')), plannedStages:Number(data.get('plannedStages')), active:true }; if (well.name.length < 2 || well.alias.length < 2 || !Number.isInteger(well.plannedStages) || well.plannedStages < 1) { toast('Enter a well name, alias, and planned stage count.'); return; } try { if (liveMode) await setDoc(doc(database, 'sites', well.siteId, 'wells', well.id), { name:well.name, alias:well.alias, color:well.color, plannedStages:well.plannedStages, active:form.dataset.id ? ((state.adminWells || []).find(item => item.id === well.id)?.active ?? true) : true, updatedAtIso:new Date().toISOString(), updatedBy:state.userName }, { merge:true }); const site = state.sites.find(item => item.id === well.siteId); const record = { ...well, siteName:site?.name || well.siteId }; const index = (state.adminWells || []).findIndex(item => item.id === well.id); if (index >= 0) state.adminWells[index] = { ...state.adminWells[index], ...record }; else state.adminWells = [...(state.adminWells || []), record]; save(); document.querySelector('.modal-backdrop')?.remove(); render(); toast(`${well.alias} is ready for CNG.`); } catch (error) { toast(error.message || 'Unable to save well.'); } });
