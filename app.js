...
const themeStorageKey = 'fieldops-color-theme';
...
function applyTheme(nextTheme) { theme = nextTheme; document.documentElement.dataset.theme = theme; document.documentElement.style.colorScheme = theme; localStorage.setItem(themeStorageKey, theme); }
applyTheme(theme);
...
const WELL_COLORS = { red:'#a91e22', white:'#ffffff', blue:'#205c8d', orange:'#b87516', black:'#262626', yellow:'#d29a00', green:'#147a5b', purple:'#7c5aa6', teal:'#397a82' };
function wellColorValue(color) { const value = String(color || '').toLowerCase(); return WELL_COLORS[value] || (/^#[0-9a-f]{6}$/i.test(value) ? value : '#7c7b7a'); }
function wellColorLabel(color) { const value = String(color || '').toLowerCase(); return WELL_COLORS[value] ? `${value[0].toUpperCase()}${value.slice(1)}` : 'Custom'; }
function wellTextColor(color) { const value = wellColorValue(color).replace('#', ''); const red = parseInt(value.slice(0, 2), 16); const green = parseInt(value.slice(2, 4), 16); const blue = parseInt(value.slice(4, 6), 16); return red * 0.299 + green * 0.587 + blue * 0.114 > 165 ? '#262626' : '#ffffff'; }
...
function themeButton(gate = false) { return `<button class="theme-toggle ${gate ? 'gate-theme-toggle' : ''}" data-theme-toggle type="button" aria-label="Switch to ${theme === 'dark' ? 'light' : 'dark'} mode">${theme === 'dark' ? '☀ Light' : '☾ Night'}</button>`; }
function gate() {
  if (state.authMode === 'admin') return `<section class="overlay">${themeButton(true)}<form class="gate" id="admin-gate"><div class="gate-mark">FO</div><h2>Administrator sign in</h2><p>Use your FieldOps administrator email and password to manage worksite access.</p><label class="field">EMAIL<input required name="email" type="email" autocomplete="email" /></label><label class="field">PASSWORD<input required name="password" type="password" autocomplete="current-password" /></label><button class="primary" type="submit">Sign in as administrator</button><button class="link-button auth-switch" type="button" data-site-login>← Back to Site Code</button><div class="app-version">FieldOps ${APP_VERSION} · Field beta</div></form></section>`;
  return `<section class="overlay">${themeButton(true)}<form class="gate gate-site" id="site-gate"><div class="gate-site-top"><div class="gate-mark">FO</div><h2>Welcome to FieldOps</h2><p>Enter the Site Code for this work location. You will only see data assigned to that site.</p></div><label class="field">SITE CODE<input required name="siteCode" autocomplete="off" placeholder="Enter site code" /></label><label class="field">YOUR NAME <input required name="userName" autocomplete="name" placeholder="Used on strap history" /></label><button class="primary" type="submit">Enter FieldOps</button><button class="link-button auth-switch" type="button" data-admin-login>Administrator sign in</button><div class="gate-note">${liveMode ? 'Your Site Code is checked securely before FieldOps opens.' : 'Demo mode accepts any Site Code.'}</div><aside class="gate-qr"><img src="/ops-fracplotter-qr.png" alt="QR code to open FieldOps" /><strong>Scan to open FieldOps</strong><span>Use your phone camera.</span></aside><div class="app-version">FieldOps ${APP_VERSION} · Field beta</div></form></section>`;
}

function shell() { return `<header class="topbar"><div class="brand">FieldOps <small>Worksite Operations · ${APP_VERSION}</small></div>${state.isAdmin ? '<div class="admin-label">Administrator console</div>' : `<nav class="tabs" aria-label="FieldOps sections"><button data-tab="rundown" class="${state.tab === 'rundown' ? 'active' : ''}">T-Belt RunDown</button><button data-tab="chemicals" class="${state.tab === 'chemicals' ? 'active' : ''}">Chemicals</button><button data-tab="cng" class="${state.tab === 'cng' ? 'active' : ''}">CNG</button><button data-tab="requisitions" class="${state.tab === 'requisitions' ? 'active' : ''}">Requisitions</button></nav>`}<div class="site-actions"><span class="site-chip">${state.isAdmin ? 'System Admin' : `Location: ${escapeHtml(state.siteName)} · ID: ${escapeHtml(state.siteId)}`}</span>${themeButton()}<button class="sign-out" data-sign-out type="button">Sign out</button></div></header>`; }
...
function cng() {
  ...
  const wellOptions = cngWells().map(item => `<option value="${escapeHtml(item.id)}" ${item.id === well.id ? 'selected' : ''}>${escapeHtml(wellColorLabel(item.color))} · ${escapeHtml(item.name)}</option>`).join('');
  const stageControls = `<div class="cng-well-controls cng-stage-controls" style="--well-color:${wellColorValue(well.color)};--well-text:${wellTextColor(well.color)}"><label>ACTIVE WELL<select data-cng-well>${wellOptions}</select></label><label class="stage-inline-field">STAGE NUMBER<span><input required form="end-stage-form" name="stageNumber" type="number" min="1" step="1" value="${stageNumber}" /><small>of ${well.plannedStages}</small></span></label><button class="secondary" data-stage-history type="button">Stage history</button></div>`;
  const latestReading = trailerId => state.cng.readings.filter(reading => reading.trailerId === trailerId).reduce((latest, reading) => !latest || new Date(reading.recordedAtIso) > new Date(latest.recordedAtIso) ? reading : latest, null);
  const readingRows = trailers.map(trailer => { const latest = latestReading(trailer.id); const summary = latest ? `<span class="last-reading"><small>Last reading</small><b>${escapeHtml(latest.pressurePsi)} PSI${latest.temperatureF === null || latest.temperatureF === undefined ? '' : ` · ${escapeHtml(latest.temperatureF)}°F`}</b><em>${escapeHtml(formatDateTime(latest.recordedAtIso))}</em></span>` : '<span class="last-reading muted">No prior reading</span>'; return `<div class="cng-reading-row"><div class="trailer-reading-info"><b><span class="position">${trailer.position}</span> Trailer ${escapeHtml(trailer.trailerNumber)}</b>${summary}</div><div class="cng-entry-fields"><label><span>PSI</span><input name="psi-${trailer.id}" data-cng-psi="${trailer.id}" type="number" min="0" step="1" inputmode="decimal" placeholder="PSI" /></label><label><span>°F</span><input name="temp-${trailer.id}" data-cng-temp="${trailer.id}" type="number" step="0.1" inputmode="decimal" placeholder="°F" /></label></div></div>`; }).join('');
  const readings = trailers.length ? `<form id="cng-readings-form"><div class="cng-reading-list">${readingRows}</div><div class="cng-save-row"><span>New readings appear under each trailer after saving.</span><button class="primary" type="submit">Save readings</button></div></form>` : '<div class="empty">No active trailers. Use Manage trailers to add the location lineup.</div>';
  return `<section class="cng-page"><div class="heading"><div><div class="title-row"><h1>CNG</h1></div><p class="subhead">Quick trailer readings and end-stage gas volume.</p></div></div><div class="cng-layout"><section class="panel cng-readings"><div class="panel-heading"><strong>Location trailer readings</strong><div><span>PSI required · °F optional</span><button class="link-button" data-manage-trailers type="button">Manage trailers</button></div></div>${readings}</section><section class="panel cng-stage"><div class="panel-heading"><strong>End stage</strong><span>MSCF pulled</span></div>${stageControls}<form id="end-stage-form" class="cng-stage-form"><label class="field">MSCF PULLED THIS STAGE<input required name="mscf" type="number" min="0" step="0.1" inputmode="decimal" placeholder="Enter total MSCF" /></label><label class="field">NOTE <span class="optional">Optional</span><textarea name="note" maxlength="280" placeholder="Stage-end note"></textarea></label><button class="cng-end-button" type="submit">Review end stage</button><p class="cng-help">The end time is saved automatically. Volume after this entry rolls into the next stage.</p></form></section></div><section class="panel cng-trend"><div class="panel-heading"><strong>Trailer pressure trend</strong><div class="cng-trend-controls"><span>Stage-end markers</span><div class="range-selector">${[12,24,36,48].map(hours => `<button data-cng-range="${hours}" class="${state.cng.rangeHours === hours ? 'active' : ''}" type="button">${hours}h</button>`).join('')}</div></div></div>${cngPressureChart(trailers)}</section><p class="cng-footnote">${completed} of ${well.plannedStages} stages completed for ${escapeHtml(well.name)}.</p></section>`;
}

function cngPressureChart(trailers) {
  const cutoff = Date.now() - state.cng.rangeHours * 60 * 60 * 1000;
  const readings = state.cng.readings.filter(reading => Number(reading.pressurePsi) >= 0 && new Date(reading.recordedAtIso).getTime() >= cutoff);
  if (!readings.length) return '<div class="empty">Trailer pressure trends will appear after readings are saved for this location.</div>';
  ...
  return `<div class="cng-chart-wrap"><div class="cng-chart-legend">${legend}</div><svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Location trailer pressure trend"><title>Trailer pressure in PSI</title>${grid}${markers}${paths}<text x="${width / 2}" y="${height - 10}" text-anchor="middle">Last ${state.cng.rangeHours} hours</text><text x="13" y="${height / 2}" transform="rotate(-90 13 ${height / 2})">PSI</text></svg></div>`;
}
...
function wellModal(id, selectedSiteId) { const well = (state.adminWells || []).find(item => item.id === id); const siteId = well?.siteId || selectedSiteId; const selectedSite = state.sites.find(site => site.id === siteId); const title = well ? `Edit ${well.name}` : 'Add well'; const presets = ['red','white','blue','orange','black','yellow']; const selectedColor = presets.includes(well?.color) ? well.color : 'other'; const customColor = selectedColor === 'other' ? wellColorValue(well?.color || '#7C3AED') : '#7C3AED'; const locationField = siteId ? `<label class="field">LOCATION<span class="locked-location">${escapeHtml(selectedSite?.name || siteId)} · ${escapeHtml(siteId)}</span><input type="hidden" name="siteId" value="${escapeHtml(siteId)}" /></label>` : `<label class="field">LOCATION<select required name="siteId">${state.sites.filter(site => site.active).map(site => `<option value="${escapeHtml(site.id)}" ${well?.siteId === site.id ? 'selected' : ''}>${escapeHtml(site.name)} · ${escapeHtml(site.id)}</option>`).join('')}</select></label>`; const colorChoices = presets.map(color => `<button class="well-color-choice ${selectedColor === color ? 'selected' : ''}" type="button" data-well-color-choice="${color}" aria-pressed="${selectedColor === color}" style="--well-swatch:${wellColorValue(color)}"><i aria-hidden="true"></i><span>${color[0].toUpperCase() + color.slice(1)}</span></button>`).join(''); return modal(`<h2>${escapeHtml(title)}</h2><p>Wells are available to CNG users at their assigned location.</p><form id="well-form" data-id="${escapeHtml(well?.id || '')}">${locationField}<label class="field">WELL NAME<input required name="name" value="${escapeHtml(well?.name || '')}" placeholder="Example: Anthem 14-23H" /></label><div class="field well-color-field"><span>WELL COLOR</span><input type="hidden" name="color" data-selected-well-color value="${selectedColor}" /><div class="well-color-picker" role="group" aria-label="Well color">${colorChoices}<label class="well-color-choice custom-color-choice ${selectedColor === 'other' ? 'selected' : ''}" style="--well-swatch:${escapeHtml(customColor)}"><input name="customColor" data-custom-well-color type="color" value="${escapeHtml(customColor)}" aria-label="Choose a custom well color" /><i aria-hidden="true"></i><span>Custom</span></label></div><small data-well-color-value>Selected: ${selectedColor === 'other' ? escapeHtml(customColor.toUpperCase()) : escapeHtml(wellColorLabel(selectedColor))}</small></div><label class="field">PLANNED STAGES<input required name="plannedStages" type="number" min="1" step="1" value="${escapeHtml(well?.plannedStages || '')}" placeholder="42" /></label><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">${well ? 'Save well' : 'Add well'}</button></div></form>`); }
function manageWellsModal(siteId) { const site = state.sites.find(item => item.id === siteId); const wells = (state.adminWells || []).filter(well => well.siteId === siteId); return modal(`<h2>${escapeHtml(site?.name || siteId)} wells</h2><p>Set up and manage the wells assigned to this location.</p><div class="manage-wells-list">${wells.length ? wells.map(well => `<div class="manage-well-item"><div><b><span class="well-dot" style="background:${wellColorValue(well.color)}"></span>${escapeHtml(well.name)}</b><span>${well.plannedStages} planned stages · ${well.active ? 'Active' : 'Inactive'}</span></div><button class="link-button" type="button" data-manage-edit-well="${escapeHtml(well.id)}">Edit</button></div>`).join('') : '<div class="muted">No wells have been added to this location yet.</div>'}</div><div class="modal-actions"><button type="button" class="secondary" data-close>Done</button><button class="primary" type="button" data-add-well-for="${escapeHtml(siteId)}">+ Add well</button></div>`); }
...
function endStageConfirmationModal(data) { return modal(`<h2>End Stage ${data.stageNumber}?</h2><p>Confirm this stage-end record before it is saved.</p><div class="confirmation-list"><div><span>WELL</span><b class="well-confirmation" style="background:${wellColorValue(data.well.color)};color:${wellTextColor(data.well.color)}"><i aria-hidden="true"></i>${escapeHtml(data.well.name)}</b></div><div><span>MSCF PULLED</span><b>${data.mscf}</b></div><div><span>END TIME</span><b>${escapeHtml(formatDateTime(data.endedAtIso))}</b></div></div><form id="confirm-end-stage-form" data-stage="${data.stageNumber}" data-mscf="${data.mscf}" data-note="${escapeHtml(data.note)}" data-ended-at="${data.endedAtIso}"><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">Confirm end stage</button></div></form>`); }
...
function bind() {
  document.querySelectorAll('[data-theme-toggle]').forEach(button => button.addEventListener('click', () => { applyTheme(theme === 'dark' ? 'light' : 'dark'); render(); }));
  ...
  document.querySelectorAll('[data-manage-wells]').forEach(button => button.addEventListener('click', () => { app.insertAdjacentHTML('beforeend', manageWellsModal(button.dataset.manageWells)); bindModal(); }));
...
function bindModal() {
  ...
  const setWellColor = color => { const selected = document.querySelector('[data-selected-well-color]'); const customInput = document.querySelector('[data-custom-well-color]'); if (!selected || !customInput) return; selected.value = color; document.querySelectorAll('[data-well-color-choice], .custom-color-choice').forEach(choice => { const isSelected = choice.dataset.wellColorChoice === color || (color === 'other' && choice.classList.contains('custom-color-choice')); choice.classList.toggle('selected', isSelected); choice.setAttribute('aria-pressed', String(isSelected)); }); const value = document.querySelector('[data-well-color-value]'); if (value) value.textContent = `Selected: ${color === 'other' ? customInput.value.toUpperCase() : wellColorLabel(color)}`; };
  document.querySelectorAll('[data-well-color-choice]').forEach(button => button.addEventListener('click', () => setWellColor(button.dataset.wellColorChoice)));
  document.querySelector('[data-custom-well-color]')?.addEventListener('input', event => { event.currentTarget.closest('.custom-color-choice').style.setProperty('--well-swatch', event.currentTarget.value); setWellColor('other'); });
  document.querySelectorAll('[data-add-well-for]').forEach(button => button.addEventListener('click', () => { document.querySelector('.modal-backdrop')?.remove(); app.insertAdjacentHTML('beforeend', wellModal('', button.dataset.addWellFor)); bindModal(); }));
  document.querySelectorAll('[data-manage-edit-well]').forEach(button => button.addEventListener('click', () => { const well = (state.adminWells || []).find(item => item.id === button.dataset.manageEditWell); document.querySelector('.modal-backdrop')?.remove(); app.insertAdjacentHTML('beforeend', wellModal(button.dataset.manageEditWell, well?.siteId)); bindModal(); }));
  document.querySelector('#well-form')?.addEventListener('submit', async event => { event.preventDefault(); const form = event.currentTarget; const data = new FormData(form); const selectedColor = String(data.get('color')); const customColor = String(data.get('customColor') || '').trim(); const color = selectedColor === 'other' ? customColor : selectedColor; const well = { id:form.dataset.id || crypto.randomUUID(), siteId:String(data.get('siteId')), name:String(data.get('name')).trim(), color, plannedStages:Number(data.get('plannedStages')), active:true }; if (well.name.length < 2 || !/^(red|white|blue|orange|black|yellow)$/i.test(well.color) && !/^#[0-9a-f]{6}$/i.test(well.color) || !Number.isInteger(well.plannedStages) || well.plannedStages < 1) { toast('Enter a well name, valid color, and planned stage count.'); return; } try { if (liveMode) await setDoc(doc(database, 'sites', well.siteId, 'wells', well.id), { name:well.name, color:well.color, plannedStages:well.plannedStages, active:form.dataset.id ? ((state.adminWells || []).find(item => item.id === well.id)?.active ?? true) : true, updatedAtIso:new Date().toISOString(), updatedBy:state.userName }, { merge:true }); const site = state.sites.find(item => item.id === well.siteId); const record = { ...well, siteName:site?.name || well.siteId }; const index = (state.adminWells || []).findIndex(item => item.id === well.id); if (index >= 0) state.adminWells[index] = { ...state.adminWells[index], ...record }; else state.adminWells = [...(state.adminWells || []), record]; save(); document.querySelector('.modal-backdrop')?.remove(); render(); toast(`${well.name} is ready for CNG.`); } catch (error) { toast(error.message || 'Unable to save well.'); } });
