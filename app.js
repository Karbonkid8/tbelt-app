...
function endStageConfirmationModal(data) { return modal(`<h2>End Stage ${data.stageNumber}?</h2><p>Confirm this stage-end record before it is saved.</p><div class="confirmation-list"><div><span>WELL</span><b>${escapeHtml(data.well.name)}</b></div><div><span>MSCF PULLED</span><b>${data.mscf}</b></div><div><span>END TIME</span><b>${escapeHtml(formatDateTime(data.endedAtIso))}</b></div></div><form id="confirm-end-stage-form" data-stage="${data.stageNumber}" data-mscf="${data.mscf}" data-note="${escapeHtml(data.note)}" data-ended-at="${data.endedAtIso}"><div class="modal-actions"><button type="button" class="secondary" data-close>Cancel</button><button class="primary" type="submit">Confirm end stage</button></div></form>`); }
