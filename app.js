...
function cng() {
  ...
  const wellOptions = cngWells().map(item => `<option value="${escapeHtml(item.id)}" ${item.id === well.id ? 'selected' : ''}>${escapeHtml(item.name)}</option>`).join('');
  const stageControls = `<div class="cng-well-controls cng-stage-controls well-selection-${escapeHtml(well.color)}"><label>ACTIVE WELL<select data-cng-well>${wellOptions}</select></label><span class="stage-progress well-status-${escapeHtml(well.color)}"><i aria-hidden="true"></i><span><b>${escapeHtml(well.name)}</b><small>Stage ${stageNumber} of ${well.plannedStages}</small></span></span><button class="secondary" data-stage-history type="button">Stage history</button></div>`;
