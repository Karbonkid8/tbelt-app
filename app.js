...
function gate() {
  ...
  return `<section class="overlay"><form class="gate gate-site" id="site-gate"><div class="gate-site-top"><div class="gate-mark">FO</div><h2>Welcome to FieldOps</h2><p>Enter the Site Code for this work location. You will only see data assigned to that site.</p></div><label class="field">SITE CODE<input required name="siteCode" autocomplete="off" placeholder="Enter site code" /></label><label class="field">YOUR NAME <input required name="userName" autocomplete="name" placeholder="Used on strap history" /></label><button class="primary" type="submit">Enter FieldOps</button><button class="link-button auth-switch" type="button" data-admin-login>Administrator sign in</button><div class="gate-note">${liveMode ? 'Your Site Code is checked securely before FieldOps opens.' : 'Demo mode accepts any Site Code.'}</div><aside class="gate-qr"><img src="/ops-fracplotter-qr.png" alt="QR code to open FieldOps" /><strong>Scan to open FieldOps</strong><span>Use your phone camera.</span></aside><div class="app-version">FieldOps ${APP_VERSION} · Field beta</div></form></section>`;
}
